/*
    TokenSmith: A decentralized options protocol for Sui
    This module implements covered call and put options with the following features:
    - Fully collateralized options
    - European-style exercise (only at expiration)
    - Support for different decimals between assets
    - Built-in safety checks and time-based restrictions
...
module tokensmith::tokensmith;
    ...
    /// Represents the exercise price as a fraction to handle decimal precision
    /// numerator/denominator gives the price in USDC per unit of asset
    ...
    /// Used when converting between assets with different decimal places
    /// @param amount: The amount to adjust
    /// @param from_decimals: Source decimal places
    /// @param to_decimals: Target decimal places
    ...
    /// Verify option is expired and can be exercised
    ...
    /// Verify withdrawal period has started (7 days after expiry)
    ...
    /// Locks asset collateral and mints option tokens
    /// Option tokens have the same decimal places as the underlying asset
    ...
    /// Locks USDC collateral and mints option tokens
    /// Collateral amount is calculated based on exercise price and option amount
    ...
    /// Exercise call option after expiry
    /// Burns option tokens, transfers locked assets to exerciser
    /// and USDC payment to vault owner
    ...
    /// Exercise put option after expiry
    /// Burns option tokens, transfers USDC collateral to exerciser
    /// and delivered assets to vault owner
    ...
    /// Withdraw tokens before expiry by returning option tokens
    /// Returns corresponding collateral to option token holder
    ...
    /// For calls: claim accumulated USDC from exercises
    /// For puts: claim accumulated assets from exercises
    ...
    /// Final withdrawal after waiting period (7 days post-expiry)
    /// Allows vault owner to withdraw any remaining collateral
